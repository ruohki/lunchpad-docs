...
export const actionShots: Shot[] = [
  ...
  // The other half of the HTTP action: the answer kept as a file, and reused while it is unchanged.
  shot(
    "httpRequestFile",
    [
      act({
        ...
        headers: [{ name: "xi-api-key", value: "{{secret.elevenlabs}}" }],
        contentType: "application/json",
        body: '{\n  "text": "{{message}}"\n}',
        bodyMode: "text",
        bodyFile: null,
    ...
    { scenario: { variables: { message: "Thanks for the raid!", voiceId: "aria" } } },
  ),
  shot(
    "runScript",
    [
